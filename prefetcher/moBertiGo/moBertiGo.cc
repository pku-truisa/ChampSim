...
// Memory Object aware BertiGo prefetcher

/******************************************************************************/
/*                      Latency table functions                               */
/******************************************************************************/
uint8_t moBertiGo::LatencyTable::add(uint64_t addr, uint64_t tag, bool pf, uint64_t cycle)
{
  /*
...
uint64_t moBertiGo::LatencyTable::del(uint64_t addr)
{
  /*
  ...
  for (int i = 0; i < size; i++) {
    ...
    if (latencyt[i].addr == addr) {
      ...
      if constexpr (champsim::debug_print) {
        ...
        std::cout << " prefetch: " << +latencyt[i].pf;
...
uint64_t moBertiGo::LatencyTable::get(uint64_t addr)
{
  /*
...
uint64_t moBertiGo::LatencyTable::get_tag(uint64_t addr)
{
  /*
...
bool moBertiGo::ShadowCache::get(uint64_t addr)
{
  /*
   * Parameters:
   *      - addr: cache block v_addr
...
void moBertiGo::ShadowCache::set_pf(uint64_t addr, bool pf)
{
  /*
   * Parameters:
   *      - addr: cache block v_addr
...
/******************************************************************************/
void moBertiGo::HistoryTable::add(uint64_t tag, uint64_t addr, uint64_t cycle)
{
  /*
...
uint16_t moBertiGo::HistoryTable::get_aux(uint32_t latency, uint64_t tag, uint64_t act_addr, uint64_t* tags, uint64_t* addr, uint64_t cycle)
{
  /*
...
uint16_t moBertiGo::HistoryTable::get(uint32_t latency, uint64_t tag, uint64_t act_addr, uint64_t* tags, uint64_t* addr, uint64_t cycle)
{
  /*
   * Return an array (by parameter) with all the possible PC that can launch
   * an on-time and late prefetch
...
/******************************************************************************/
void moBertiGo::InnerBerti::increase_conf_tag(uint64_t tag)
{
  /*
...
void moBertiGo::InnerBerti::add(uint64_t tag, int64_t delta)
{
  /*
   * Save the new information into the history table
   *
...
uint8_t moBertiGo::InnerBerti::get(uint64_t tag, std::vector<delta_t>& res)
{
  /*
   * Save the new information into the history table
   *
...
void moBertiGo::InnerBerti::find_and_update(uint64_t latency, uint64_t tag, uint64_t cycle, uint64_t line_addr)
{
...
bool moBertiGo::InnerBerti::compare_rpl(delta_t a, delta_t b)
{
...
bool moBertiGo::InnerBerti::compare_greater_delta(delta_t a, delta_t b)
{
...
uint64_t moBertiGo::InnerBerti::ip_hash(uint64_t ip)
{
  /*
   * IP hash function
   */
...
/*                        Cache Functions                                     */
/******************************************************************************/
void moBertiGo::prefetcher_initialize()
{
  pf_to_l1 = 0;
  pf_to_l2 = 0;
  pf_to_l2_bc_mshr = 0;
...
#endif
#ifdef HASH_ORIGINAL
  std::cout << "moBERTI HASH ORIGINAL" << std::endl;
#endif
#ifdef THOMAS_WANG_HASH_1
  std::cout << "moBERTI HASH 1" << std::endl;
#endif
#ifdef THOMAS_WANG_HASH_2
  std::cout << "moBERTI HASH 2" << std::endl;
#endif
#ifdef THOMAS_WANG_HASH_3
  std::cout << "moBERTI HASH 3" << std::endl;
#endif
#ifdef THOMAS_WANG_HASH_4
  std::cout << "moBERTI HASH 4" << std::endl;
#endif
#ifdef THOMAS_WANG_HASH_5
  std::cout << "moBERTI HASH 5" << std::endl;
#endif
#ifdef THOMAS_WANG_HASH_6
  std::cout << "moBERTI HASH 6" << std::endl;
#endif
#ifdef THOMAS_WANG_HASH_7
  std::cout << "moBERTI HASH 7" << std::endl;
#endif
#ifdef THOMAS_WANG_NEW_HASH
  std::cout << "moBERTI HASH NEW" << std::endl;
#endif
#ifdef THOMAS_WANG_HASH_HALF_AVALANCHE
  std::cout << "moBERTI HASH HALF AVALANCHE" << std::endl;
#endif
#ifdef THOMAS_WANG_HASH_FULL_AVALANCHE
  std::cout << "moBERTI HASH FULL AVALANCHE" << std::endl;
#endif
#ifdef THOMAS_WANG_HASH_INT_1
  std::cout << "moBERTI HASH INT 1" << std::endl;
#endif
#ifdef THOMAS_WANG_HASH_INT_2
  std::cout << "moBERTI HASH INT 2" << std::endl;
#endif
#ifdef ENTANGLING_HASH
  std::cout << "moBertiGo HASH ENTANGLING" << std::endl;
#endif
#ifdef FOLD_HASH
  std::cout << "moBERTI HASH FOLD" << std::endl;
#endif
  std::cout << "moBertiGo IP MASK " << std::hex << IP_MASK << std::dec << std::endl;
  std::cout << "moBertiGo L1D Conf: " << std::dec << CONFIDENCE_L1 << " L2C: " << CONFIDENCE_L2 << " L2R: " << CONFIDENCE_L2R << std::endl;

  std::cout << "moBertiGo Parameters" << std::endl;
  // Sizes
  std::cout << "  SIZES: "
...
uint32_t moBertiGo::prefetcher_cache_operate(champsim::address address, champsim::address ip_addr, uint8_t cache_hit, bool useful_prefetch,
                                             access_type type, uint32_t metadata_in)
{
  ...
  {
    if constexpr (champsim::debug_print)
      std::cout << "[moBERTI] operate cache miss" << std::endl;
  ...
  {
    if constexpr (champsim::debug_print)
      std::cout << "[moBERTI] operate cache hit because of pf" << std::endl;
  ...
  for (auto i : deltas) {
    ...
    if (prefetch_line(p_addr, fill_this_level, 0)) {
      ...
      if constexpr (champsim::debug_print) {
        std::cout << "[moBERTI] operate prefetch delta: " << i.delta;
...
uint32_t moBertiGo::prefetcher_cache_fill(champsim::address address, long set, long way, uint8_t prefetch, champsim::address evicted_address,
                                          uint32_t metadata_in)
{
  ...
  if constexpr (champsim::debug_print) {
    std::cout << "[moBERTI] fill addr: " << std::hex << line_addr;
...
void moBertiGo::prefetcher_cycle_operate() {}

void moBertiGo::prefetcher_final_stats()
{
  std::cout << "moBERTI "
            << "TO_L1: " << pf_to_l1 << " TO_L2: " << pf_to_l2;
  std::cout << " TO_L2_BC_MSHR: " << pf_to_l2_bc_mshr << std::endl;

  std::cout << "moBERTI AVG_LAT: ";
  ...
  std::cout << "moBERTI CROSS_PAGE " << cross_page;
