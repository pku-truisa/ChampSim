//=======================================================================================//
// File             : moPythia/moPythia_helper.cc
...
void Scooby_STEntry::update(uint64_t _page, uint64_t pc, uint32_t offset, uint64_t address)
{
  ...
  /* insert PC */
  if (this->pcs.size() >= MO_PYTHIA::max_pcs) {
    this->pcs.pop_front();
  }
  ...
  if (!this->offsets.empty()) {
    ...
    if (this->deltas.size() >= MO_PYTHIA::max_deltas) {
  ...
  /* insert offset */
  if (this->offsets.size() >= MO_PYTHIA::max_offsets) {
    this->offsets.pop_front();
  }
  this->offsets.push_back(offset);
}

void Scooby_STEntry::update_alloc_id(uint64_t _page, uint64_t alloc_id, uint32_t offset, uint64_t address)
{
  assert(this->page == _page);

  /* insert alloc_id */
  if (this->alloc_ids.size() >= MO_PYTHIA::max_alloc_ids) {
    this->alloc_ids.pop_front();
  }
...
uint32_t Scooby_STEntry::get_object_sig()
{
  uint32_t signature = 0;

  /* compute signature using last 4 alloc_ids */
  uint32_t n = (uint32_t)alloc_ids.size();
  uint32_t ptr = (n >= 4) ? (n - 4) : 0;
...
void Scooby_STEntry::insert_action_tracker(int32_t pref_offset)
{
  ...
  if (it != action_tracker.end()) {
    ...
    if (action_tracker.size() >= MO_PYTHIA::action_tracker_size) {
