...
moFeatureKnowledge::moFeatureKnowledge(FeatureType feature_type, float alpha, float gamma, uint32_t actions, uint32_t num_tilings, uint32_t num_tiles,
...
moFeatureKnowledge::~moFeatureKnowledge() {}

float moFeatureKnowledge::getQ(uint32_t tiling, uint32_t tile_index, uint32_t action)
{
...
void moFeatureKnowledge::setQ(uint32_t tiling, uint32_t tile_index, uint32_t action, float value)
{
  assert(tiling < m_num_tilings);
  assert(tile_index < m_num_tiles);
  assert(action < m_actions);
...
float moFeatureKnowledge::retrieveQ(State* state, uint32_t action)
{
...
void moFeatureKnowledge::updateQ(State* state1, uint32_t action1, int32_t reward, State* state2, uint32_t action2)
{
...
uint32_t moFeatureKnowledge::get_tile_index(uint32_t tiling, State* state)
{
  ...
  case F_Object:
    return process_Object(tiling, state->alloc_id);
  case F_Object_Delta:
...
uint32_t moFeatureKnowledge::getMaxAction(State* state)
{
...
std::string moFeatureKnowledge::get_feature_string(State* state)
{
  uint64_t pc = state->pc;
  int32_t delta = state->delta;
...
//----------------------------------------------------//
uint32_t moFeatureKnowledge::process_PC(uint32_t tiling, uint64_t pc)
{
...
uint32_t moFeatureKnowledge::process_offset(uint32_t tiling, uint32_t offset)
{
  if (m_enable_tiling_offset)
...
uint32_t moFeatureKnowledge::process_delta(uint32_t tiling, int32_t delta)
{
...
uint32_t moFeatureKnowledge::process_address(uint32_t tiling, uint64_t address)
{
...
uint32_t moFeatureKnowledge::process_PC_offset(uint32_t tiling, uint64_t pc, uint32_t offset)
{
  uint64_t tmp = pc;
  tmp = tmp << 6;
  tmp += offset;
...
uint32_t moFeatureKnowledge::process_PC_address(uint32_t tiling, uint64_t pc, uint64_t address)
{
  uint64_t tmp = pc;
  tmp = tmp << 16;
  tmp ^= address;
...
uint32_t moFeatureKnowledge::process_PC_page(uint32_t tiling, uint64_t pc, uint64_t page)
{
  uint64_t tmp = pc;
  tmp = tmp << 16;
  tmp ^= page;
...
uint32_t moFeatureKnowledge::process_PC_path(uint32_t tiling, uint32_t pc_path)
{
  if (m_enable_tiling_offset)
...
uint32_t moFeatureKnowledge::process_delta_path(uint32_t tiling, uint32_t delta_path)
{
  if (m_enable_tiling_offset)
...
uint32_t moFeatureKnowledge::process_offset_path(uint32_t tiling, uint32_t offset_path)
{
  if (m_enable_tiling_offset)
...
uint32_t moFeatureKnowledge::process_PC_delta(uint32_t tiling, uint64_t pc, int32_t delta)
{
  uint32_t unsigned_delta = (delta < 0) ? (((-1) * delta) + (1 << (DELTA_BITS - 1))) : delta;
  uint64_t tmp = pc;
  tmp = tmp << 7;
...
uint32_t moFeatureKnowledge::process_PC_offset_delta(uint32_t tiling, uint64_t pc, uint32_t offset, int32_t delta)
{
  uint32_t unsigned_delta = (delta < 0) ? (((-1) * delta) + (1 << (DELTA_BITS - 1))) : delta;
  uint64_t tmp = pc;
  tmp = tmp << 6;
...
uint32_t moFeatureKnowledge::process_Page(uint32_t tiling, uint64_t page)
{
...
uint32_t moFeatureKnowledge::process_PC_Path_Offset(uint32_t tiling, uint32_t pc_path, uint32_t offset)
{
...
uint32_t moFeatureKnowledge::process_PC_Path_Offset_Path(uint32_t tiling, uint32_t pc_path, uint32_t offset_path)
{
  uint64_t tmp = pc_path;
  tmp = tmp << 16;
  tmp += offset_path;
...
uint32_t moFeatureKnowledge::process_PC_Path_Delta(uint32_t tiling, uint32_t pc_path, int32_t delta)
{
  uint32_t unsigned_delta = (delta < 0) ? (((-1) * delta) + (1 << (DELTA_BITS - 1))) : delta;
  uint64_t tmp = pc_path;
  tmp = tmp << 7;
...
uint32_t moFeatureKnowledge::process_PC_Path_Delta_Path(uint32_t tiling, uint32_t pc_path, uint32_t delta_path)
{
  uint64_t tmp = pc_path;
  tmp = tmp << 16;
  tmp += delta_path;
...
uint32_t moFeatureKnowledge::process_PC_Path_Offset_Path_Delta_Path(uint32_t tiling, uint32_t pc_path, uint32_t offset_path, uint32_t delta_path)
{
  uint64_t tmp = pc_path;
  tmp = tmp << 10;
  tmp ^= offset_path;
...
uint32_t moFeatureKnowledge::process_Offset_Path_PC(uint32_t tiling, uint32_t offset_path, uint64_t pc)
{
...
uint32_t moFeatureKnowledge::process_Delta_Path_PC(uint32_t tiling, uint32_t delta_path, uint64_t pc)
{
...
uint32_t moFeatureKnowledge::process_Object(uint32_t tiling, uint64_t alloc_id)
{
  uint32_t raw_index = folded_xor(alloc_id, 2); /* 32-b folded XOR */
  if (m_enable_tiling_offset)
    raw_index = raw_index ^ tiling_offset[tiling];
  uint32_t hashed_index = HashZoo::getHash(m_hash_type, raw_index);
  return (hashed_index % m_num_tiles);
...
uint32_t moFeatureKnowledge::process_Object_delta(uint32_t tiling, uint64_t alloc_id, int32_t delta)
{
  uint32_t unsigned_delta = (delta < 0) ? (((-1) * delta) + (1 << (DELTA_BITS - 1))) : delta;
  uint64_t tmp = alloc_id;
  tmp = tmp << 7;
  tmp += unsigned_delta;
  uint32_t raw_index = folded_xor(tmp, 2);
  if (m_enable_tiling_offset)
