...
void moPythia::init_knobs()
{
  ...
  assert(Actions.size() == MO_PYTHIA::actions.size());
  assert(Actions.size() <= MO_PYTHIA::max_actions);
  assert(MO_PYTHIA::last_pref_offset_conf_thresholds.size() == MO_PYTHIA::dyn_degrees_type2.size() - 1);
...
mop::Scooby_STEntry* moPythia::update_local_state(uint64_t pc, uint64_t page, uint32_t offset, uint64_t address)
{
  ...
  auto st_index = find_if(signature_table.begin(), signature_table.end(), [page](mop::Scooby_STEntry* _stentry) { return _stentry->page == page; });
                           ...
                           bool has_object_bounds, uint64_t obj_start, uint64_t obj_end)
{
  ...
  if (MO_PYTHIA::enable_dyn_degree) {
  ...
  if (Actions[action_index] != 0) {
    ...
    if (is_out_of_bounds) /* falls outside the allowed bounds */
    {
      ...
      if (MO_PYTHIA::enable_reward_out_of_bounds) {
        addr = 0xdeadbeef;
...
bool moPythia::track(uint64_t address, mop::State* state, uint32_t action_index, mop::Scooby_PTEntry** tracker)
{
  ...
  bool new_addr = true;
  std::vector<mop::Scooby_PTEntry*> ptentries = search_pt(address, false);
  if (ptentries.empty()) {
    new_addr = true;
  } else {
    new_addr = false;
  ...
  assert(prefetch_tracker.size() <= MO_PYTHIA::pt_size);
...
uint32_t moPythia::get_dyn_pref_degree(float max_to_avg_q_ratio, uint64_t page, int32_t action)
{
  ...
  auto st_index = find_if(signature_table.begin(), signature_table.end(), [page](mop::Scooby_STEntry* stentry) { return stentry->page == page; });
  if (st_index != signature_table.end()) {
    int32_t conf = 0;
    bool found = (*st_index)->search_action_tracker(action, conf);
    std::vector<int32_t> conf_thresholds, deg_normal;

    conf_thresholds = high_bw ? MO_PYTHIA::last_pref_offset_conf_thresholds_hbw : MO_PYTHIA::last_pref_offset_conf_thresholds;
    deg_normal = high_bw ? MO_PYTHIA::dyn_degrees_type2_hbw : MO_PYTHIA::dyn_degrees_type2;
...
void moPythia::reward(uint64_t address)
{
  ...
  std::vector<mop::Scooby_PTEntry*> ptentries = search_pt(address, MO_PYTHIA::enable_reward_all);
  ...
  for (uint32_t index = 0; index < ptentries.size(); ++index) {
    mop::Scooby_PTEntry* ptentry = ptentries[index];
    ...
    {
      assign_reward(ptentry, mop::RewardType::correct_timely);
    } else {
      assign_reward(ptentry, mop::RewardType::correct_untimely);
    }
...
{
  bool high_bw = (MO_PYTHIA::enable_hbw_reward && is_high_bw(get_dram_bw())) ? true : false;
  int32_t reward = 0;

  stats.reward.compute_reward.dist[type][high_bw]++;

  if (type == mop::RewardType::correct_timely) {
    reward = high_bw ? MO_PYTHIA::reward_hbw_correct_timely : MO_PYTHIA::reward_correct_timely;
  } else if (type == mop::RewardType::correct_untimely) {
    reward = high_bw ? MO_PYTHIA::reward_hbw_correct_untimely : MO_PYTHIA::reward_correct_untimely;
  } else if (type == mop::RewardType::incorrect) {
    reward = high_bw ? MO_PYTHIA::reward_hbw_incorrect : MO_PYTHIA::reward_incorrect;
  } else if (type == mop::RewardType::none) {
    reward = high_bw ? MO_PYTHIA::reward_hbw_none : MO_PYTHIA::reward_none;
  } else if (type == mop::RewardType::out_of_bounds) {
    reward = high_bw ? MO_PYTHIA::reward_hbw_out_of_bounds : MO_PYTHIA::reward_out_of_bounds;
  } else {
    assert(false);
  }
...
bool moPythia::is_high_bw(uint8_t bw_level) { return bw_level >= MO_PYTHIA::high_bw_thresh ? true : false; }