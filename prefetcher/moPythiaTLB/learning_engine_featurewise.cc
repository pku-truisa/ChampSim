...
{
  assert(MO_PYTHIA_TLB::le_featurewise_active_features.size() == MO_PYTHIA_TLB::le_featurewise_num_tilings.size());
  assert(MO_PYTHIA_TLB::le_featurewise_active_features.size() == MO_PYTHIA_TLB::le_featurewise_num_tiles.size());
  assert(MO_PYTHIA_TLB::le_featurewise_active_features.size() == MO_PYTHIA_TLB::le_featurewise_enable_tiling_offset.size());
}
    ...
    : moLearningEngineBase(alpha, gamma, epsilon, actions, 0 /*dummy state value*/, seed, policy, type)
{
  ...
  for (uint32_t index = 0; index < MO_PYTHIA_TLB::le_featurewise_active_features.size(); ++index) {
    assert(MO_PYTHIA_TLB::le_featurewise_active_features[index] < NumFeatureTypes);
    m_feature_knowledges[MO_PYTHIA_TLB::le_featurewise_active_features[index]] = new moFeatureKnowledge(
        (FeatureType)MO_PYTHIA_TLB::le_featurewise_active_features[index], alpha, gamma, actions, MO_PYTHIA_TLB::le_featurewise_num_tilings[index],
        MO_PYTHIA_TLB::le_featurewise_num_tiles[index], MO_PYTHIA_TLB::le_featurewise_hash_types[index], MO_PYTHIA_TLB::le_featurewise_enable_tiling_offset[index]);
    assert(m_feature_knowledges[MO_PYTHIA_TLB::le_featurewise_active_features[index]]);
  }

  m_max_q_value = (double)1 / (1 - gamma) * (double)std::accumulate(MO_PYTHIA_TLB::le_featurewise_num_tilings.begin(), MO_PYTHIA_TLB::le_featurewise_num_tilings.end(), 0);
...
float moLearningEngineFeaturewise::consultQ(State* state, uint32_t action)
{
  ...
  for (uint32_t index = 0; index < NumFeatureTypes; ++index) {
    if (m_feature_knowledges[index]) {
      if (MO_PYTHIA_TLB::le_featurewise_pooling_type == 1) /* sum pooling */
      {
        q_value += m_feature_knowledges[index]->retrieveQ(state, action);
      } else if (MO_PYTHIA_TLB::le_featurewise_pooling_type == 2) /* max pooling */
      {
