    ...
    for (uint64_t i = 1; i < (REGION_SIZE / BLOCK_SIZE); i++) {
        ...
        if (pf_offset != trigger && pf_offset != second && pattern[pf_offset] != 0) {
            ...
            if (cache->get_pq_occupancy().back() + cache->get_mshr_occupancy() < cache->get_mshr_size() - 1 && cache->get_pq_occupancy().back() < cache->get_pq_size().back()) {
                ...
                bool ok = cache->prefetch_line(champsim::address{pf_addr}, pattern[pf_offset] == PF_FILL_L1 ? true : false, pf_metadata);
...
void gaze::prefetcher_initialize()
{
  std::cout << intern_->NAME << " Gaze prefetcher" << std::endl;
}

uint32_t gaze::prefetcher_cache_operate(champsim::address addr, champsim::address ip, uint8_t cache_hit,
                                        bool useful_prefetch, access_type type, uint32_t metadata_in)
{
  if (type != access_type::LOAD)
    return metadata_in;

  uint64_t block_num = addr.to<uint64_t>() >> LOG2_BLOCK_SIZE;
  uint64_t pc = ip.to<uint64_t>();

  impl.access(block_num, pc, intern_);
  impl.prefetch(intern_, block_num);

  return metadata_in;
}

uint32_t gaze::prefetcher_cache_fill(champsim::address addr, long set, long way, uint8_t prefetch,
