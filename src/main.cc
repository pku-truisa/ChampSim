...
int main(int argc, char** argv) // NOLINT(bugprone-exception-escape)
{
  ...
  VirtualMemory* stlb_vmem = nullptr;
  for (PageTableWalker& ptw : gen_environment.ptw_view()) {
    if (ptw.vmem != nullptr) {
      ptw.vmem->set_mol_table(mol_table);
      if (stlb_vmem == nullptr) {
        stlb_vmem = ptw.vmem;
      }
    }
  }

  // Wire the STLB for the RTLB fast path (recent large-object translations). The DTLB is
  // intentionally left unwired, so it keeps its original 4KB behavior.
  if (large_object_allocation_enabled && stlb_vmem != nullptr) {
    for (CACHE& cache : gen_environment.cache_view()) {
      if (cache.NAME.find("STLB") != std::string::npos) {
        cache.set_mol_table(mol_table);
        cache.set_vmem(*stlb_vmem);
      }
    }
  }
