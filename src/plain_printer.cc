...
std::vector<std::string> champsim::plain_printer::format(CACHE::stats_type stats)
{
  ...
  for (auto cpu : cpus) {
    ...
    lines.push_back(fmt::format("cpu{}->{} PREFETCH REQUESTED: {:10} ISSUED: {:10} USEFUL: {:10} USELESS: {:10} FILL: {:10}", cpu, stats.name, stats.pf_requested,
                                stats.pf_issued, stats.pf_useful, stats.pf_useless, stats.pf_fill));
