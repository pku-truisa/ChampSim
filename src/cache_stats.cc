...
static PerCacheStats* mol_lookup_cache(champsim::address pa, const std::string& cache_name)
{
...
void cache_stats::record_hit(access_type type, uint32_t cpu, champsim::address pa, bool warmup)
{
  hits.increment(std::pair{type, cpu});
  if (!warmup) {
    auto* st = mol_lookup_cache(pa, name);
    if (st) {
...
void cache_stats::record_miss(access_type type, uint32_t cpu, champsim::address pa, bool warmup)
{
...
void cache_stats::record_mshr_merge(access_type type, uint32_t cpu, champsim::address pa, bool warmup)
{
...
void cache_stats::record_mshr_return(access_type type, uint32_t cpu, champsim::address pa, bool warmup)
{
...
void cache_stats::record_miss_latency(champsim::address pa, long latency, bool warmup)
{
...
void cache_stats::record_pf_requested(champsim::address pa, bool warmup)
{
  ++pf_requested;
  if (!warmup) {
    auto* st = mol_lookup_cache(pa, name);
...
void cache_stats::record_pf_issued(champsim::address pa, bool warmup)
{
  ++pf_issued;
  if (!warmup) {
    auto* st = mol_lookup_cache(pa, name);
...
void cache_stats::record_pf_useful(champsim::address pa, bool warmup)
{
  ++pf_useful;
  if (!warmup) {
    auto* st = mol_lookup_cache(pa, name);
...
void cache_stats::record_pf_useless(champsim::address pa, bool warmup)
{
  ++pf_useless;
  if (!warmup) {
    auto* st = mol_lookup_cache(pa, name);
...
void cache_stats::record_pf_fill(champsim::address pa, bool warmup)
{
  ++pf_fill;
  if (!warmup) {
    auto* st = mol_lookup_cache(pa, name);
